...
def callback(dashApp, fromToTimestampFunction = None, states = []):
    refreshInterval.callback(dashApp)
    states.insert(0, State(component_id = "fromTimestampInput", component_property = "value"))
    @dashApp.callback([Output(component_id = "fromTimestampInput", component_property = "value"),
        ...
        states)
    def callback(urlHref, lastFiveMinutesLiNClicks, lastFifthteenMinutesLiNClicks, lastThirtyMinutesLiNClicks, lastOneHourLiNClicks,
        ...
        thisYearSoFarLiNClicks, intervalNIntervals, fromTimestampInputValue, *args):
        ...
        if fromToTimestampFunction is not None:
            timestamps = fromToTimestampFunction(fromTimestamp, toTimestamp, *args)
...
def layout():
    ...
    [
        html.Div(id = "fromToTimestampsDiv", className = "btn-group", role = "group", children =
        [
            "From: ", 
        ...
        html.Div(id = "quickTimeRangePickerDiv", className = "btn-group", role = "group", children =
        [
