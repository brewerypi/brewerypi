...
CREATE DEFINER=`pi`@`%` PROCEDURE `spElementSummary`(IN attributeTemplateNames TEXT, IN elementIds TEXT)
BEGIN
    ...
    WHERE AttributeTemplate.ElementTemplateId = elementTemplateId AND
		FIND_IN_SET(AttributeTemplate.Name, attributeTemplateNames) > 0;
